...
API_EXPORTED int fpi_img_to_print_data_bang(struct fp_dev *dev, struct fp_img *img,
	struct fp_print_data **ret)
{
	struct fp_print_data *print;
	struct fp_print_data_item *item;
...
API_EXPORTED int fpi_compare_data_bang(unsigned char *enrolled_data, unsigned char *new_data) {
  ...
  enrolled_print = fp_print_data_from_data(enrolled_data, 12050);
  new_print = fp_print_data_from_data(new_data, 2418);
  /* return new_print->type; */
